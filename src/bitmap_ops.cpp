...
#define GET_RED(c)    ((((uint32_t) c) >> 16) & 0xFF)
#define GET_GREEN(c)  ((((uint32_t) c) >> 8) & 0xFF)
#define GET_BLUE(c)   (((uint32_t) c) & 0xFF)

void clear_window_buffer(uint8_t r, uint8_t g, uint8_t b)
{
    for (uint32_t i = 0; i < NES_WIDTH * NES_HEIGHT; i++)
    {
        nsp::window_buffer[i] = MFB_RGB(r, g, b);
    }
}
...
void dimm_window_buffer(float dimm)
{
    for (uint32_t i = 0; i < NES_WIDTH * NES_HEIGHT; i++)
    {
        ...
        uint8_t r = GET_RED(nsp::window_buffer[i])*dimm;
        uint8_t g = GET_GREEN(nsp::window_buffer[i])*dimm;
        uint8_t b = GET_BLUE(nsp::window_buffer[i])*dimm;
        nsp::window_buffer[i] = MFB_RGB(r, g, b);
    }
}
...
{
    for (uint32_t y0 = y; y0 < y+h; ++y0)
    {
        for (uint32_t x0 = x; x0 < x+w; ++x0)
        {
            uint32_t i = y0*NES_WIDTH + x0;
            nsp::window_buffer[i] = MFB_RGB(r, g, b);
        }
    }
}
...
void draw_glyph(uint32_t x, uint32_t y, uint8_t glyph)
{
    for (uint32_t gy = 0; gy < 8; ++gy)
    {
        for (uint32_t gx = 0; gx < 8; ++gx)
        {
            ...
            {
                nsp::window_buffer[bi] = MFB_RGB(255, 255, 255);
            }
        }
    }
}
